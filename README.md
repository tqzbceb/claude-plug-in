...
面板内的输入框 / 下拉框统一加高到约 `2.25 × 主题字号`（默认约 34px，原生 25px），
按钮行矮一点、约 `1.85 ×`（默认约 28px），比输入框矮但不再是扁条。
「新建 / 删除 / 小眼睛」跟同行的输入框等高，手机上点得准。
高度是拿 `--mainFontSize` 算的，改主题字号会一起缩放；
所有样式都只在 `#ttal_root` 里生效，关掉插件立刻回到原生尺寸。
...
- 尺寸实测：下拉框 / 预设名称 / 端点 / 密钥 / 模型名 / 模型列表 / 小眼睛 / 新建 / 删除 全部 34px 等高，
  六行整体宽度都是同一个值；按钮行 28px；关掉插件后端点框回到原生 25px
